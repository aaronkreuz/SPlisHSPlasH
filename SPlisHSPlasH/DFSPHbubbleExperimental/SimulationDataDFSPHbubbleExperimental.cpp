#include "SimulationDataDFSPHbubbleExperimental.h"
#include "SPlisHSPlasH/SPHKernels.h"
#include "SPlisHSPlasH/Simulation.h"

using namespace SPH;

SimulationDataDFSPHbubbleExperimental::SimulationDataDFSPHbubbleExperimental() :
	m_factor(),
	m_pressure_rho2(),
	m_pressure_rho2_V(),
	m_pressureAccel(),
...
SimulationDataDFSPHbubbleExperimental::~SimulationDataDFSPHbubbleExperimental(void)
{
	cleanup();
}


void SimulationDataDFSPHbubbleExperimental::init()
{
	...
	for (unsigned int i = 0; i < nModels; i++)
	{
		...
		if(sim->getFluidModel(i)->getId() == "Air"){
			...
			m_lifetimeAir.resize(fm->numParticles(), 0.7f); // in paper: 0.7 sec.
		}
	}
}

void SimulationDataDFSPHbubbleExperimental::cleanup()
{
	Simulation *sim = Simulation::getCurrent();
	const unsigned int nModels = sim->numberOfFluidModels();
...
void SimulationDataDFSPHbubbleExperimental::reset()
{
	Simulation *sim = Simulation::getCurrent();
	const unsigned int nModels = sim->numberOfFluidModels();
...
void SimulationDataDFSPHbubbleExperimental::performNeighborhoodSearchSort()
{
	Simulation *sim = Simulation::getCurrent();
	const unsigned int nModels = sim->numberOfFluidModels();
...
void SimulationDataDFSPHbubbleExperimental::emittedParticles(FluidModel *model, const unsigned int startIndex)
{
	Simulation* sim = Simulation::getCurrent();
