#include "TimeStepPCISPHbubble.h"
#include "SPlisHSPlasH/TimeManager.h"
#include "SPlisHSPlasH/SPHKernels.h"
#include "SimulationDataPCISPHbubble.h"
#include <iostream>
...
TimeStepPCISPHbubble::TimeStepPCISPHbubble() :
	TimeStep()
{
...
TimeStepPCISPHbubble::~TimeStepPCISPHbubble(void)
{
	Simulation *sim = Simulation::getCurrent();
	const unsigned int nModels = sim->numberOfFluidModels();
	for (unsigned int fluidModelIndex = 0; fluidModelIndex < nModels; fluidModelIndex++)
...
void TimeStepPCISPHbubble::step()
{
	Simulation *sim = Simulation::getCurrent();
...
void TimeStepPCISPHbubble::pressureSolve()
{
	Simulation *sim = Simulation::getCurrent();
...
void TimeStepPCISPHbubble::pressureSolveIteration(const unsigned int fluidModelIndex, Real &avg_density_err)
{
	Simulation *sim = Simulation::getCurrent();
	FluidModel *model = sim->getFluidModel(fluidModelIndex);
	const int numParticles = (int)model->numActiveParticles();
...
void TimeStepPCISPHbubble::reset()
{
	TimeStep::reset();
...
void TimeStepPCISPHbubble::performNeighborhoodSearch()
{
...
void TimeStepPCISPHbubble::emittedParticles(FluidModel *model, const unsigned int startIndex)
{
	m_simulationData.emittedParticles(model, startIndex);
}

void TimeStepPCISPHbubble::resize()
{
	m_simulationData.init();
}
